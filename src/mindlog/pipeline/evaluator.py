"""
Extraction pipeline evaluation — computes classification metrics comparing
LLM extraction results against ground-truth annotations.

Metrics:
  Per field: accuracy, precision (macro), recall (macro), F1 (macro),
             Cohen's Kappa
  Aggregate: overall macro-averaged accuracy, exact-match rate,
...
def _cohens_kappa(y_true: list, y_pred: list, labels: list) -> float:
    """
    Compute Cohen's Kappa — inter-rater agreement corrected for chance.

    κ = (p_o - p_e) / (1 - p_e)
    where p_o = observed agreement, p_e = expected agreement by chance.

    Returns a value in [-1, 1]. 1 = perfect agreement, 0 = chance agreement.
    """
    n = len(y_true)
    if n == 0:
        return 0.0
...
def generate_evaluation_report(
    ...
    # ── Error Analysis ────────────────────────────────────────
    error_analysis = _compile_error_analysis(extractions, ground_truths, fields)
...
def format_results_table(report: dict) -> pd.DataFrame:
    """
    Format evaluation results as a DataFrame:
